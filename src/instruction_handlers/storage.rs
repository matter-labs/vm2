...
fn sstore(vm: &mut VirtualMachine, args: &Arguments, world: &mut dyn World) {
...
fn sstore_transient(vm: &mut VirtualMachine, args: &Arguments, _world: &mut dyn World) {
    let key = Register1::get(args, &mut vm.state);
    let value = Register2::get(args, &mut vm.state);
...
fn sload(vm: &mut VirtualMachine, args: &Arguments, world: &mut dyn World) {
    let key = Register1::get(args, &mut vm.state);
    let (value, refund) = vm
        .world_diff
...
fn sload_transient(vm: &mut VirtualMachine, args: &Arguments, _world: &mut dyn World) {
    let key = Register1::get(args, &mut vm.state);
    let value = vm
        .world_diff
...
impl Instruction {
    ...
    pub fn from_sstore(src1: Register1, src2: Register2, arguments: Arguments) -> Self {
        Self {
            handler: Handler::Sequential(sstore),
...
impl Instruction {
    ...
    pub fn from_sstore_transient(src1: Register1, src2: Register2, arguments: Arguments) -> Self {
        Self {
            handler: Handler::Sequential(sstore_transient),
            arguments: arguments.write_source(&src1).write_source(&src2),
        }
    }
}
...
impl Instruction {
    ...
    pub fn from_sload(src: Register1, dst: Register1, arguments: Arguments) -> Self {
        Self {
            handler: Handler::Sequential(sload),
...
impl Instruction {
    ...
    pub fn from_sload_transient(src: Register1, dst: Register1, arguments: Arguments) -> Self {
        Self {
            handler: Handler::Sequential(sload_transient),
            arguments: arguments.write_source(&src).write_destination(&dst),
        }
    }
}
