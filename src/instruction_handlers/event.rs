...
fn event(vm: &mut VirtualMachine, args: &Arguments, _world: &mut dyn World) {
...
fn l2_to_l1(vm: &mut VirtualMachine, args: &Arguments, _world: &mut dyn World) {
    let key = Register1::get(args, &mut vm.state);
    let value = Register2::get(args, &mut vm.state);
...
impl Instruction {
    pub fn from_event(
        ...
        Self {
            handler: Handler::Sequential(event),
            arguments: arguments
                .write_source(&key)
                .write_source(&value)
    ...
    pub fn from_l2_to_l1_message(
        ...
        Self {
            handler: Handler::Sequential(l2_to_l1),
            arguments: arguments
                .write_source(&key)
                .write_source(&value)
